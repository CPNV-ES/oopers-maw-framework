...
abstract class AbstractField
{
    ...
    public static function createFromFormBuilder(
        string $property,
        string $type,
        object $entity,
        array $options = []
    ): self {
        if (!is_subclass_of($type, AbstractField::class)) {
            throw new FormException("Unable to use `$type` as form field.");
        }
        $propertyReflection = new ReflectionProperty($entity, $property);
        $camelCase = str_replace(['_'], [''], ucwords($property, "\t\r\n\f\v_"));
        if (!$propertyReflection->getDeclaringClass()->hasMethod('get' . $camelCase)) {
            throw new FormException(
                "Unable to find getter for $property in {$propertyReflection->getDeclaringClass()->getName()}"
            );
        }
        if (!$propertyReflection->getDeclaringClass()->hasMethod('set' . $camelCase)) {
            throw new FormException(
                "Unable to find setter for $property in {$propertyReflection->getDeclaringClass()->getName()}"
            );
        }

        return (new $type(
            $property,
            $propertyReflection->getDeclaringClass()->getMethod('get' . $camelCase)->invoke($entity),
            $propertyReflection,
            $entity
        ))
            ->setName($property)
    ...
    {
        if ($this->getEntityValue() === $this->getValue() || $this->hasError()) {
            return $this;
        }
        $this
